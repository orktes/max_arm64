...
static int ffmpeg_available =
    -1; // -1 = not checked, 0 = not available, 1 = available
...
int videoplayer_is_available(void) { return check_ffmpeg_availability(); }
...
int videoplayer_play(const char *filename) {
  ...
  if (ffmpeg_pid == -1) {
    ...
    // The drawtext filter reads from the overlay file and positions text at
    // bottom left
    snprintf(scale_filter, sizeof(scale_filter), "scale=%d:%d", screen_width,
             screen_height);
    snprintf(overlay_filter, sizeof(overlay_filter),
             "%s,drawtext=textfile=%s:fontcolor=white:fontsize=24:x=10:y=h-th-"
             "10:reload=1",
    ...
    execl("/usr/bin/ffmpeg", "ffmpeg", "-i", input_path, "-pix_fmt", "bgra",
          "-vf", overlay_filter, "-f", "fbdev", "/dev/fb0", NULL);

    // If execl fails, try from PATH
    execlp("ffmpeg", "ffmpeg", "-i", input_path, "-pix_fmt", "bgra", "-vf",
           overlay_filter, "-f", "fbdev", "/dev/fb0", NULL);
    ...
    if (result == ffmpeg_pid) {
      ...
      debugPrintf(
          "videoplayer: ffmpeg process exited immediately with status %d\n",
          WEXITSTATUS(status));
      ffmpeg_pid = -1;
      return -1;
    } else if (result == -1) {
