...
local function handleAxis(dt, ui, order)
  ...
  if ui.mode == "config" then
    repeatLogic('x', lx, 
      function() if callbacks.adjustValue then callbacks.adjustValue(order[ui.sel], -1) end end,
      function() if callbacks.adjustValue then callbacks.adjustValue(order[ui.sel], 1) end end)
  end
  repeatLogic('y', ly, 
    function() if callbacks.moveSel then callbacks.moveSel(-1) end end,
    function() if callbacks.moveSel then callbacks.moveSel(1) end end)
end
    ...
    if keyName ~= "repeatDelay" and keyName ~= "repeatRate" and keyData.held then
      ...
      if keyData.timer >= keyRepeat.repeatDelay then
        ...
        if keyName == "left" and ui.mode == "config" then
          if callbacks.adjustValue then callbacks.adjustValue(order[ui.sel], -1) end
        elseif keyName == "right" and ui.mode == "config" then
          if callbacks.adjustValue then callbacks.adjustValue(order[ui.sel], 1) end
        elseif keyName == "up" then
          if callbacks.moveSel then callbacks.moveSel(-1) end
...
function input.gamepadPressed(joy, button, ui, launcherOptions)
  ...
  if ui.mode == "launcher" then
    ...
    elseif button == "a" then
      if ui.sel == 1 then
        if callbacks.launchGame then callbacks.launchGame() end
      elseif ui.sel == 2 then
        if callbacks.enterSettings then callbacks.enterSettings() end
      end
    elseif button == "b" or button == "back" then 
      if callbacks.exitApp then callbacks.exitApp() end
    end
    return
  end
  ...
  if ui.edit then
    ...
    if button == "b" or button == "back" then 
      if callbacks.exitEdit then callbacks.exitEdit() end
  ...
  elseif button == "dpleft" then 
    if callbacks.adjustValue then callbacks.adjustValue(ui.order[ui.sel], -1) end
  elseif button == "dpright" then 
    if callbacks.adjustValue then callbacks.adjustValue(ui.order[ui.sel], 1) end
  elseif button == "a" then 
    if callbacks.toggleValue then callbacks.toggleValue(ui.order[ui.sel]) end
  elseif button == "y" then 
    if callbacks.loadConfig then callbacks.loadConfig() end
  elseif button == "start" then 
    if callbacks.saveConfig then callbacks.saveConfig() end
  elseif button == "back" or button == "b" then 
    if callbacks.saveAndExit then callbacks.saveAndExit() end
  end
end
...
function input.keyPressed(key, ui, launcherOptions)
  ...
  elseif key == "e" and ui.order and ui.order[ui.sel] == "mod_file" then 
    if callbacks.enterEdit then callbacks.enterEdit() end
  end
end
